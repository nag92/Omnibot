...
class EKF(object):
    ...
    def update(self,u ):
        ...
        self.mu = self.eval_model(u, self.state, self.g)
        self.G_eval = np.matrix( self.eval_model(u, self.state, self.G))
        return self.mu
        
    def predict(self, z):
        #make the state into a list
        temp = np.array(self.mu.T)[0].tolist()
        ...
        #do the EKF 
        cov  = self.G_eval*(self.sigma)*self.G_evalT + self.R
    ...
    def new_H(h,R):
        self.h = h #sensor function,a list if lamnda fns
        self.H = [nd.Jacobian(f) for f in self.h  ]
        self.R = R #measurement noise 
